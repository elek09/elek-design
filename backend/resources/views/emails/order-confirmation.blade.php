...
<body>
<div class="container">
    <h1>{{ $isQuote ? 'Köszönjük az árajánlat kérését!' : 'Köszönjük a rendelését!' }}</h1>

    @php($isModel = $order instanceof \App\Models\Order)

    @if($isModel)
        <p>Kedves {{ $order->customer_name ?? 'Vásárló' }},</p>
        <p>{{ $isQuote ? 'Az alábbi ajánlatot rögzítettük:' : 'Az alábbi rendelést rögzítettük:' }}</p>
        ...
        @endif
    @else
        {{-- Simple array payload fallback --}} 
        <p>Kedves {{ $order['name'] ?? 'Vásárló' }},</p>
        <p>{{ $isQuote ? 'Az alábbi ajánlatot rögzítettük:' : 'Az alábbi rendelést rögzítettük:' }}</p>
