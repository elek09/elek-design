...
<head>
    ...
    <style>
        ...
        a.button { display:inline-block; background:#111827; color:#fff !important; text-decoration:none; padding:8px 12px; border-radius:6px; font-weight:600; }
    </style>
</head>
<body>
<div class="container">
    <h1>Új rendelés érkezett <span class="badge">#{{ $order->id }}</span></h1>

    @php($adminBase = rtrim((string) (config('services.admin_app_url') ?: config('app.url')), '/'))
    <p>
        <a class="button" href="{{ $adminBase }}/admin/orders" target="_blank" rel="noopener">Megnyitás az Admin felületen</a>
    </p>
